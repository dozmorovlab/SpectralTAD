...
The package is based around the windowed spectral clustering algorithm, introduced by [@cresswell:2019aa] [??? Make references work], which is designed to be robust to sparsity, noise, and sequencing depth of Hi-C data. 
...
[??? When compiled, one row number is formatted like 1e+05, while the others are like 125000. Can you make them all similar in numeric notation?]
...
2. Convert to text file using `cooler dump --join Rao2014-GM12878-DpnII-allreps-filtered.50kb.cool > Rao.GM12878.50kb.txt` [??? Describe how to install cooler]
